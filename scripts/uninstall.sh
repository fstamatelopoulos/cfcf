...
# @cerefox/codefactory`. Per-platform native package + transitive
# runtime deps come along for free. User data (`~/.cfcf/clio.db`,
# `~/.cfcf/logs/`, platform config dir, workspace `cfcf-docs/`) is
# intentionally NOT touched -- removing those is a separate manual
# decision.
#
...
if ! command -v bun >/dev/null 2>&1; then
  ...
  echo "[cfcf-uninstall] or run: rm -rf \"\$HOME/.bun/install/global/node_modules/@cerefox/codefactory\""
  exit 1
fi

cat <<'EOF'
[cfcf-uninstall] About to run: bun remove -g @cerefox/codefactory
