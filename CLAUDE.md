...
- **System-managed Clio Projects** (`packages/core/src/clio/system-projects.ts`): three Clio Projects are owned by cfcf code, not the user — `cf-system-default` (auto-route fallback for workspaces without an explicit `clioProject`), `cf-system-memory-global` (cross-role global memory + user preferences), and `cf-system-pa-memory` (Product Architect per-workspace memory). Per-role memory projects (`cf-system-ha-memory`, `cf-system-reflection-memory`, etc.) join the set as their roles wire up. **Naming convention**: `cf-system-*`. **Locked from edit/delete** at the backend layer (`editProject` + `deleteProject` throw on a system-project name), so the lock holds across web UI + CLI + any future surface; the web UI also hides the buttons + shows a `(system)` badge. Names + the `SYSTEM_PROJECTS` set are the single source of truth — any code that writes to one of these projects imports the constant rather than hardcoding the string. Pre-existing data in old names (`cfcf-memory-pa`, `default`, etc.) from before the rename is reachable via the web UI's Projects tab — users can manually migrate via the per-doc project field or the per-workspace Clio Project picker.
