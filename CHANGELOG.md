...
- **`cfcf completion install`** — auto-installs the completion file for the detected shell (writes `~/.cfcf-completion.bash` for bash, `~/.zsh/completions/_cfcf` for zsh). Idempotent: subsequent runs regenerate the file silently. Trust principle: cfcf only touches files it owns, never edits user `~/.bashrc` / `~/.zshrc`. Pass `--print-only` for the manual-setup output instead of auto-write.
