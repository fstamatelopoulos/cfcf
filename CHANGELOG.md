...
Minor bump because the release adds new adapter names + a new `availableOllamaModels` field on `CfcfGlobalConfig`. Reading + writing remain backward-compatible: `validateConfig` backfills missing fields; existing per-role adapter assignments aren't touched. New adapters are opt-in via `cfcf init` / `cfcf config edit`.

### Added — Item 6.28: adapter expansion for Anthropic harness policy compliance

Three new agent adapters in `packages/core/src/adapters/`:

- **`opencode`** (direct): runs `opencode run --dangerously-skip-permissions [--model provider/model] <prompt>`. Instruction filename `AGENTS.md` (matches opencode + codex docs). Uses your own `opencode auth login` provider config; cfcf doesn't surface a model list (the picker shows "(adapter default)" + the custom-name sentinel from 6.26 instead, since opencode's auth/provider config is outside cfcf's view).
- **`claude-code-ollama`**: wraps Claude Code via `ollama launch claude --model <local-model> --yes -- --dangerously-skip-permissions -p <prompt>`. Requires both `ollama` and `claude` on PATH. Instruction filename `CLAUDE.md`. Models picker is sourced from `ollama list`. No Anthropic subscription OAuth involved — ollama implements the Anthropic Messages API surface locally and Claude Code points at `localhost:11434` via the launch wrapper.
- **`opencode-ollama`**: same `ollama launch` wrapper pattern with opencode as the wrapped agent. Instruction filename `AGENTS.md`. Models picker from `ollama list`. **Note**: opencode-ollama in cf²'s harness pattern has known UX-fragile behaviours (silent error swallowing on ollama 5xx, hardcoded permission denies that survive `--dangerously-skip-permissions` for some tool families per [anomalyco/opencode#13851](https://github.com/anomalyco/opencode/issues/13851), stdout buffering). It works end-to-end when the environment is clean (we verified SCOPE_COMPLETE return on a fully-shipped workspace), but `claude-code-ollama` is the better-tested ollama path. See `docs/guides/anthropic-policy.md` § opencode-ollama stability.

Plus **ollama detection** as a standalone helper alongside `detectAvailableAgents()`: runs `ollama --version` to confirm installation and `ollama list` to enumerate locally-pulled models. The model snapshot persists on `CfcfGlobalConfig.availableOllamaModels` and refreshes on `cfcf init` / `cfcf init --force` / `cfcf config edit`. cfcf does NOT auto-install ollama or pull models — that stays the user's responsibility; `cfcf doctor` surfaces the detected state.

### Added — Anthropic harness-policy framing (UX + docs)

- **CLI warning** in `cfcf init` and `cfcf config edit`: yellow callout when `claude-code` is picked for any unattended role (dev / judge / reflection / documenter, plus architect when `autoReviewSpecs=true`); blue info callout when `claude-code-ollama` is picked (the path is policy-clean but `claude -p` still buffers stdout — log-visibility caveat). Both are informational; neither blocks the save. PA / HA / manually-invoked SA on `claude-code` do NOT trigger either callout — within Anthropic's allowed-interactive scope.
- **Web UI warning** via the new `<HarnessPolicyWarning>` component, rendered in both Settings → Agent roles (global config) and the workspace Config tab. Same two-callout treatment as the CLI; CLI + web in lockstep.
- **`cfcf doctor`** gains two new checks: (a) "Ollama (for *-ollama adapters)" reports installation status + locally-pulled-model count; (b) "Anthropic harness policy (per-role)" reads the saved config and warns if `claude-code` is configured for any unattended role; (c) "Claude Code's `~/.claude/settings.json` model field" — heuristic that warns when the model field contains `:` (likely an ollama-flavoured name) and shows the `jq` fix command. Plus the existing agent-CLI probes now include `opencode`.
- **`cfcf help anthropic-policy`** topic registered with aliases `policy` / `harness` / `ollama`. The canonical 18KB guide at `docs/guides/anthropic-policy.md` covers the policy text + carve-outs, the role-to-adapter recommendation table, the local-ollama setup walkthrough, the `~/.claude/settings.json` side-effect of `ollama launch claude` and how to fix it, log-visibility trade-offs across adapters, and known-fragile opencode-ollama caveats.
- **Documentation sweep**: README + `docs/guides/installing.md` gain per-adapter install walkthroughs (claude-code, codex, opencode, ollama) with the explicit "claude-code alone is not enough" framing. `manual.md`, `cli-usage.md`, `workflow.md`, `troubleshooting.md` updated with cross-links + section refreshes for both the policy and log-visibility concerns.
- **Default per-role adapter preference flipped** for unattended roles when codex is detected: dev / judge / reflection / documenter prefer `codex` over `claude-code` at first-run, so a fresh `cfcf init` doesn't immediately fire the policy warning. Architect / Product Architect / Help Assistant keep `claude-code` preference (interactive scope).

### Fixed — Architect-runner robustness (item 6.28 dogfood follow-ups)

- **SA returns SCOPE_COMPLETE for finished projects.** The architect prompt template's re-review section gets an explicit decision tree: when `cfcf-docs/plan.md` has zero pending items + no new problem-pack requirements, return `SCOPE_COMPLETE` (not `READY`). Plus a defensive auto-promotion in `parseArchitectSignals`: if the agent returns `READY` but plan.md has all `[x]` and zero `[ ]`, override to `SCOPE_COMPLETE` with a one-line `[architect] note:` log entry. Pre-fix, `READY` on a finished project sent the dev agent into a wasted iteration. Surfaced when qwen3-coder ran the SA on a fully-shipped calc workspace and returned READY.
- **Architect-signals validator no longer false-rejects clean READY/SCOPE_COMPLETE verdicts.** Pre-fix, `parseArchitectSignals` rejected ANY all-empty submission as "untouched template", which conflated two genuinely different cases (agent-never-edited vs. agent-found-no-issues). The validator now keys on `readiness`: empty supporting fields are accepted when readiness is READY/SCOPE_COMPLETE, rejected only when NEEDS_REFINEMENT/BLOCKED (those genuinely demand explanation).
- **SA pause-message tailored per failure mode** (was: single generic "missing or malformed"). New `diagnoseFailedArchitectSignals` classifies the failure into `missing` / `malformed_json` / `untouched_template` / `missing_readiness` / `valid` and the pause UI tailors the message + recommended next step accordingly. The "untouched_template" variant explicitly names "agent likely hung mid-run" + recommends a more reliable adapter — exactly the case opencode-ollama hits when ollama returns 5xx silently.
- **Judge skips reflection on `should_continue=false`.** Pre-fix, qwen3-coder set `determination=SUCCESS` + `should_continue=false` (correct) but ALSO set `reflection_needed=true` (defaulting per the prompt's "when in doubt" rule). Reflection ran on a loop already ending — wasted tokens. Both halves of the fix: judge prompt template's reflection-trigger section gets an explicit decision tree placing `should_continue=false` as the first opt-out condition; `shouldRunReflection` in iteration-loop also bypasses reflection when `should_continue=false` regardless of `reflection_needed` value (defensive code-side override).

### Fixed — Process-tree kill on server shutdown (item 6.31, partial)

`cfcf server stop && cfcf server start` previously left orphan agent processes alive. Each `ollama launch <agent>` spawn was a process group (wrapper + wrapped agent) but cfcf only sent SIGTERM to the immediate child. Wrappers that didn't propagate signals left their wrapped agent (e.g. `claude` itself) running as an orphan of init, holding ollama's model serializer hostage with up-to-10-minute timed-out inference requests. Multiple orphans accumulated across server restarts during the dogfood pass; the next loop's API call queued behind them and starved. From the user's POV: loop "hangs" with no clear error.

Fix: `process-manager.ts` now spawns agents with `detached: true` (puts the spawned tree in its own process group via `setsid` on Unix), and `kill()` sends SIGTERM to `process.kill(-pid, ...)` (the whole group) followed by SIGKILL after a 1.5s grace window. `start.ts` `gracefulShutdown` now waits 2s after `killAllActiveProcesses()` so the SIGKILL grace window completes BEFORE the cfcf process exits — without this wait, the setTimeout-scheduled SIGKILL would be cancelled when the JS runtime tears down. Regression test in `process-manager.test.ts` confirms a `bash -c "sleep 30 & wait"` spawn's grandchild dies when the parent is killed.

Boot-time orphan reap (catches orphans from a previously-crashed server PID that didn't run `gracefulShutdown`) is tracked as the second half of 6.31 — not yet shipped.

### Fixed — Drive-by bugs surfaced during 6.28 dogfooding

- **CLI option-shadow** in `cfcf clio docs ingest --project X` and `cfcf clio docs edit --project X`: commander.js was attributing `--project X` to the parent `docsCmd` (which had its own `-p, --project` for the default-list action) instead of the child subcommand. Result: ingest silently used its child default `cf-system-default`; edit silently no-op-ed the project change while still applying any `--set-meta`. Pinned by 4 new regression tests in `clio-option-parsing.test.ts`. Small UX regression: `cfcf clio docs` (no subcommand) now prints help instead of acting as default-list — use `cfcf clio docs list` explicitly.
- **`~/.claude/settings.json` model side-effect from `ollama launch claude`**: the launch wrapper writes the chosen model into Claude Code's settings as an undocumented side-effect. From that point on, ANY direct `claude` invocation (including cfcf's claude-code adapter, terminal use, anywhere) defaults to that model. If the model is an ollama-flavoured name (`gemma4:31b`, `qwen3-coder:latest`), direct claude invocations break with "There's an issue with the selected model" because Anthropic's API rejects the name. New `cfcf doctor` heuristic check warns when the file's `model` field contains `:`. Documented in `anthropic-policy.md` with the `jq` fix command.
- **Test suite leaked real macOS desktop notifications.** `iteration-loop.test.ts > Loop State Persistence > startLoop persists state to disk` legitimately needed to call `startLoop()` (which is fire-and-forget) but the background loop continued past `afterEach` teardown, hit ENOENT spawning git against the now-deleted tempdir, fired `loop.failed` through the dispatcher, and by then env was restored to the user's real `~/.cfcf` — the macOS notification channel happily emitted a real desktop notification "Loop failed cfcf — persist-test". Fix: new `CFCF_DISABLE_NOTIFICATIONS=1` env gate at the dispatcher entry point, set by the leaky test in beforeEach. Defense in depth against any future test or CI context leaking real notifications. 2 new unit tests pin the env-var parsing.

### Tests

- 675 core tests (was 592 before this branch; +83 net) — all green
- New regression suites: `process-manager.test.ts` (process-tree kill), `clio-option-parsing.test.ts` (commander option-shadow), `ollama-detection.test.ts`, `architect-runner.test.ts` substantively expanded (signals validator, auto-promotion, diagnose function, plan-item counter), `iteration-loop.test.ts` (reflection bypass), `dispatcher.test.ts` (CFCF_DISABLE_NOTIFICATIONS), `agent-models.test.ts` (modelSource routing)
- End-to-end manual dogfooding on the calc test workspace covered: claude-code-ollama dev iterations 1+2 with qwen3-coder + codex judge/reflection (success); codex multi-role; opencode-ollama SA returning SCOPE_COMPLETE on a fully-shipped project (works after the process-tree kill fix shipped).

### Decisions log entries (durable rationale, see `docs/decisions-log.md`)

- 2026-05-07 — Anthropic's third-party-harness policy → adapter expansion via `ollama launch` (item 6.28). Captures the policy framing, the four options considered (incl. the rejected env-var-and-proxy approach), and five lessons.
- 2026-05-08 — `cfcf clio docs ingest/edit --project X` was silently absorbed by parent option (commander.js shadowing). The CLI option-shadow fix.
- 2026-05-08 — `claude -p --verbose` buffers stdout despite the Apr-17 commit's "live progress" claim; same-day reverted from a stream-json experiment after the JSONL log was unreadable in the web UI panel.
- 2026-05-08 (later same day) — Reverted both `--verbose` AND `--output-format stream-json` on the claude-code adapters; back to plain `claude -p`. Plus the architect-signals validator fix (readiness-conditional template detection).

### Carried over to next minor (in iter-6 plan, not blocking this release)

- **6.29** — macOS notification fix (bundle .app shim with own bundle ID; current `osascript display notification` queues under "Script Editor" rate-limit + DND batching)
- **6.30** — Documenter ollama-model heuristic in `cfcf init` (warn when documenter is on a non-coder model like `gemma4` — `gemma4:31b` produced malformed tool-result blocks during a documenter run)
- **6.31** (boot-time half) — Boot-scan reap of orphans from prior cfcf server PIDs that crashed without `gracefulShutdown` (today's ship handles graceful stop only)
- **6.32** — opencode-ollama spawn timeout + log-tail integration for live progress visibility
