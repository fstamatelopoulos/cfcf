...
### Added
- **Clio (item 5.7) — pick-equals-install reverted; init becomes config-only (2026-04-25).** The 2026-04-22 "pick-equals-install" change was double-broken: (a) `loadNow: true` only constructed the OnnxEmbedder shell -- it didn't actually trigger the HF download (download is in `OnnxEmbedder.ensurePipeline`, lazy until first `embed()`), so init printed "Clio ready" while the model was still missing from `~/.cfcf/models/`; (b) even with the fix in place, a multi-minute model download mid-`cfcf init` is the wrong UX -- init should stay conversational. Decision (2026-04-25): init writes the active-embedder DB row + saves `clio.preferredEmbedder`, but no longer downloads. Message changed to "Clio configured: ... Model files (~140 MB) will download on first Clio search / ingest." Pre-warm becomes the installer's job (5.5 + the `--with-embedder` flag noted in 6.19). Bug (a) is also fixed at the lower layer: added `Embedder.warmup()`, OnnxEmbedder implements it via `ensurePipeline()`, `installActiveEmbedder({ loadNow: true })` now actually downloads -- so `cfcf clio embedder install <name>` (which keeps `loadNow: true`) works as advertised. Net effect: `cfcf init` is fast and honest; `cfcf clio embedder install` is the explicit "download now" path; the future installer pre-warms transparently.
