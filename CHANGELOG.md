...
- **`cfcf completion bash`** + **`cfcf completion zsh`** — emit static shell completion scripts derived from the commander.js command tree at script-generation time. No runtime dependency on `cfcf` per tab press (fast: no Bun cold-start). Re-run after upgrading cfcf to pick up new verbs.
- **`cfcf completion install`** — print shell-specific install instructions (detects `$SHELL`, falls back to showing both bash + zsh forms).
- **Source of truth = commander tree.** No hardcoded verb list to keep in sync — the generator walks `program.commands` recursively, so adding a new verb anywhere in the CLI flows automatically into the next-generated completion script.
...
- 9 unit tests in `packages/cli/src/commands/completion.test.ts` cover tree-walking, deduped+sorted flags, autogenerated `help` filtering, and bash/zsh script structure.
