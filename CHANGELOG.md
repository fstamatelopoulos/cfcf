...
### Added
- **Clio (item 5.7) — post-review refinements (2026-04-23).**
  - **`cfcf init` now prompts for embedder install.** The flow lists the catalogue (default marked), accepts a numeric pick or `S` to skip. We don't actually install during `cfcf init` (the install call needs the running server + the HF download takes ~30-60s, both of which `init` can't guarantee); instead the user's choice is surfaced as a "Next steps" line: either `cfcf clio embedder install <name>` (with the MB size) when they picked one, or a heads-up that Clio is in FTS-only mode when they skipped. Keeps `cfcf init` predictable + offline-safe while still nudging users toward hybrid search.
