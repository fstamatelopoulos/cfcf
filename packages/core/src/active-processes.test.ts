...
describe("active-processes registry", () => {
  ...
  test("getActiveProcessesForWorkspace returns only matching project", () => {
    const p1Dev = makeFakeProcess();
    const p1Judge = makeFakeProcess();
    const p2Dev = makeFakeProcess();
    registerProcess({ workspaceId: "p1", role: "dev", process: p1Dev, startedAt: "" });
    registerProcess({ workspaceId: "p1", role: "judge", process: p1Judge, startedAt: "" });
    registerProcess({ workspaceId: "p2", role: "dev", process: p2Dev, startedAt: "" });

    const p1 = getActiveProcessesForWorkspace("p1");
    expect(p1).toHaveLength(2);
    const p2 = getActiveProcessesForWorkspace("p2");
  ...
  test("getAllActiveProcesses returns all", () => {
    ...
    registerProcess({ workspaceId: "p1", role: "dev", process: p1Dev, startedAt: "" });
    registerProcess({ workspaceId: "p2", role: "judge", process: p2Judge, startedAt: "" });
    expect(getAllActiveProcesses()).toHaveLength(2);
  });

  test("killWorkspaceProcesses kills and removes only that project's entries", () => {
    const p1Dev = makeFakeProcess();
    const p1Judge = makeFakeProcess();
    const p2Dev = makeFakeProcess();
    registerProcess({ workspaceId: "p1", role: "dev", process: p1Dev, startedAt: "" });
    ...
    const killed = killWorkspaceProcesses("p1");
    expect(killed).toBe(2);
    expect(p1Dev.killCount).toBe(1);
    expect(p1Judge.killCount).toBe(1);
    expect(p2Dev.killCount).toBe(0);
    expect(getActiveProcessesForWorkspace("p1")).toHaveLength(0);
    expect(getActiveProcessesForWorkspace("p2")).toHaveLength(1);
  });

  test("killAllActiveProcesses kills and empties the registry", () => {
    const a = makeFakeProcess();
    const b = makeFakeProcess();
    registerProcess({ workspaceId: "p1", role: "dev", process: a, startedAt: "" });
    registerProcess({ workspaceId: "p2", role: "judge", process: b, startedAt: "" });
  ...
  test("kill() that throws is swallowed so remaining processes still get killed", () => {
    ...
    registerProcess({ workspaceId: "p1", role: "dev", process: throwing, startedAt: "" });
    registerProcess({ workspaceId: "p1", role: "judge", process: good, startedAt: "" });
