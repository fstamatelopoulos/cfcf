...
- cfcf-docs/iteration-history.md (regenerated by cfcf each iteration)
- cfcf-docs/judge-assessment.md
- cfcf-docs/user-feedback.md

## Iteration Log Format

Write `cfcf-docs/iteration-logs/iteration-N.md` (where N is the current iteration number, e.g. `iteration-3.md`) at the end of the iteration. Keep it tight -- one page. Format:
...
This line gets prepended to iteration-history.md.
...
### Removed
- `path/to/old.ts` -- why it's gone

## Tests
- Ran: `bun test`
- Passed: 178 / 180
- Failed: 2 (test_session_expiry, test_concurrent_refresh) -- see handoff

## Commits
- a1b2c3d -- feat(auth): add tokenRefresh module
- e4f5g6h -- refactor(api): delegate token handling

## Plan items closed
- [x] Iteration 3, item 1: "Implement token refresh"
```

This file is committed with your iteration's changes. It serves three purposes: (a) cfcf rebuilds `iteration-history.md` by reading `Summary` sections from these files; (b) the judge and future reflection role consume it as curated history; (c) it survives loop-restart because it lives in git.

The difference between this file and `iteration-handoff.md`: **iteration-log is backward-looking** (what you did), **iteration-handoff is forward-looking** (what's next / open questions / blockers).

## Decision-Log Format

When you append to `cfcf-docs/decision-log.md`, use the tagged format (details in the header of that file):

```markdown
## 2026-04-18T12:34:56Z  [role: dev]  [iter: 3]  [category: decision]

<one paragraph or bullet list explaining the decision and its rationale>
```

Use `[category: decision]` for choices you made, `[category: lesson]` for things you learned the hard way. Don't log routine progress here -- that goes in `plan.md` notes and the iteration-log.
...
Run tests before completing the iteration. Document results in the handoff and in the iteration-log.
