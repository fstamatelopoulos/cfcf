...
| 6.19 | 🔄 | Clio v2 — `cfcf init` Clio onboarding polish | **Partially shipped 2026-04-22:** pick-equals-install (the embedder chosen during `cfcf init` now downloads + activates inline rather than just saving a preference — removes the "why did I pick if I still have to run install?" redundancy), `cfcf clio embedder install` arg made optional (defaults to `clio.preferredEmbedder`), stderr progress bar during HuggingFace download via `progress_callback` (one-line-per-file, ≥5% ticks). **Still pending:** bandwidth warning before starting the download, post-install verification (`cfcf clio embedder active` runs automatically and prints a "ready" line), pre-warm during installer (5.5). |
