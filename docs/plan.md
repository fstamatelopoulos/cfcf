...
| 5.11 | ❌ | Clio — update-doc API + versioning + soft-delete (Cerefox parity, **prerequisite for 5.5b**) | **Promoted from 6.17 on 2026-04-26.** Today `LocalClio.ingest()` dedups by `content_hash` and silently returns `created: false` if you re-ingest byte-identical content; modified content creates a brand-new UUID and orphans the old doc with no version chain. This is the show-stopper for cf² agents using Clio the way Cerefox is used: every "the agent updated the same logical document" produces a new file instead of a new version, growing the corpus indefinitely with stale duplicates. **Scope**: (a) extend `IngestRequest` + `cfcf clio ingest` + `POST /api/clio/ingest` with `--update-if-exists` and `--document-id <uuid>` flags mirroring `cerefox_ingest`; (b) `clio_document_versions` writes — snapshot current chunks on every content update, bump `version_number`, archive prior chunks by setting `version_id` to the new version row; (c) `cfcf clio versions <document-id>` + `GET /api/clio/documents/:id/versions` surfacing the history (mirrors `cerefox_list_versions`); (d) `deleted_at` reads everywhere + `DELETE /api/clio/documents/:id` (sets it) + `POST /api/clio/documents/:id/restore` (clears it). **Reuse plan**: port Cerefox's `cerefox_update_document_content` + `cerefox_soft_delete_document` + `cerefox_restore_document` + version-number bump logic. **Why iter-5**: the `MemoryBackend` parity commitment (decisions-log 2026-04-25) means the public agent-facing semantics MUST match Cerefox before we publish 5.5b. Letting agents drive Clio without an update verb makes the corpus untrustworthy. |
