...
| 6.13 | ❌ | Cron-like recurring execution research | Research and design a scheduling layer so cfcf can fire a project on a recurring cadence -- e.g. "every weekday at 08:00 run a fresh loop against the updated Problem Pack and produce a report." Pairs directly with 6.11: for continuous research, periodic content production (newsletters, digests), scheduled code-health audits, or any pipeline where the artifact is produced repeatedly against an evolving input. Questions: Per-project cron expressions vs. per-trigger schedules? Should a recurring run always be a fresh iteration loop, a `cfcf reflect` pass, an architect re-review, or a new Problem Pack ingest? How do completed loops interact with the next scheduled run -- reset to main? carry state? How do we surface schedule history in the web UI? Does cfcf run its own scheduler (macOS `launchd` / systemd timers / `node-cron`) or integrate with the user's existing one? What guardrails prevent runaway costs if a schedule fires while the previous run is still active? Output: a research doc under `docs/research/` enumerating options + recommended design, not an implementation. |
...
| 2026-04-19 | Plan item 6.13 added: cron-like recurring execution research | Scheduling layer for repeat runs (periodic reports, continuous research, scheduled audits). Pairs directly with 6.11 (generalization to non-coding work) since recurring execution is the delivery mechanism for most non-coding use-cases. Research doc first, implementation later |
