...
| 5.5 | ❌ | Installer script | `curl -fsSL https://cerefox.org/install \| bash`. Detects platform, downloads from GitHub Releases, verifies checksum. Depends on 5.3 + 5.4 so the single binary is self-sufficient. **Also needs to handle Clio's native-binding deps** (item 5.7 2026-04-23 decision): installing alongside the binary a pinned SQLite build + the `sqlite-vec` loadable extension per-platform, so `cfcf clio` gets a consistent SQLite across macOS/Linux/Windows regardless of the system default. This bypasses the Bun-on-macOS `loadExtension`-disabled issue and gives sqlite-vec a straight path. The installer script is the natural place: "download binary → download pinned SQLite + sqlite-vec → verify checksums → wire `Database.setCustomSQLite(...)` via a known env var (e.g. `CFCF_SQLITE_LIB`)." Embedder model is still lazy-downloaded on first use per 5.7 design; SQLite + sqlite-vec come with the install because they're the load-bearing infrastructure. |
