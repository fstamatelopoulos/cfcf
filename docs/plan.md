...
| 4.24 | ✅ | Per-iteration plan execution prompt | `process.md` (dev agent) and `cfcf-architect-instructions.md` (architect) now explicitly instruct "one phase per iteration": the architect maps phases to concrete iterations in `cfcf-docs/plan.md`, and the dev agent executes the **next pending** iteration only, marks completed items `[x]` with brief notes, and exits so cfcf can judge / pause / resume between chunks. Discovered empirically while running the tracker example with a user hint -- captured and promoted into the core prompts so every project gets the checkpointed-iteration behavior by default. `plan.md` template comment now models the same structure. |
