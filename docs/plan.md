...
| 5.5b | ❌ | Publish to npmjs.com (after iter-5 stabilises) | When cfcf goes OSS / public: register `@cerefox` org on npmjs.com (✅ done 2026-04-26), run `bun publish` from release.yml, update `install.sh` default `CFCF_BASE_URL`. Install becomes the canonical `bun install -g @cerefox/cfcf-cli`. Tarball-via-GitHub-Releases stays as a fallback for offline / pinned installs. **Blockers cleared**: 5.11/5.12/5.13 (Clio Cerefox parity) shipped on `iteration-5/clio-update-api` 2026-04-26; only 5.8 (user manual) remains before publishing. |
