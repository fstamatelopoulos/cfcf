...
| 6.20 | ❌ | New-version notification in the web UI | The CLI `cfcf self-update` command shipped in 5.5 (see [`research/installer-design.md`](../research/installer-design.md) §10.2). What's left for 6.20 is the **proactive notification** layer: the server polls the releases endpoint at a low cadence (e.g. daily) and surfaces a banner on the top bar when a newer version is available, with a "How to update" tooltip pointing at `cfcf self-update`. Notification only — does NOT auto-apply updates (the user still runs the upgrade explicitly via the CLI). High-level requirement at this stage; full design when scheduled. Pairs with 6.18 (web UI Clio tab) since the same top-bar surface holds both. |
