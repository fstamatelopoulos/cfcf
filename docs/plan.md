...
| 5.5b | ❌ | Publish to npmjs.com (after iter-5 stabilises) | When cfcf goes OSS / public: register `@cerefox` org on npmjs.com (✅ done 2026-04-26), run `bun publish` from release.yml, update `install.sh` default `CFCF_BASE_URL`. Install becomes the canonical `bun install -g @cerefox/cfcf-cli`. Tarball-via-GitHub-Releases stays as a fallback for offline / pinned installs. **Acceptance**: (a) end-to-end smoke of the rewritten `release.yml` workflow — never exercised since the 5.5 rewrite. Run `gh workflow run release.yml -f tag=v0.X.Y` against a test tag, verify the build-native matrix + build-cli + release jobs all succeed and produce the expected artefacts (cfcf-X.Y.Z.tgz, three native tarballs, SHA256SUMS, MANIFEST.txt, install.sh on the GitHub Release). (b) Publish to npmjs.com under `@cerefox/cfcf-cli`. (c) Update `install.sh` defaults. **Blockers cleared**: 5.11/5.12/5.13 (Clio Cerefox parity) shipped on `iteration-5/clio-update-api` 2026-04-26; 5.8 (user manual) remains. |
...
| 5.8 | 🟡 | User manual (embedded + CLI + doc) + CLI verb normalisation + tab completion | A canonical user manual covering the full workflow (setup → problem pack → review → run → reflect → document → extend). Four sub-PRs on `iteration-5/cli-verb-normalisation` and follow-up branches. **PR1** ✅ (2026-04-27, v0.12.0): CLI verb normalisation — proposal in [`docs/research/cli-verb-normalisation.md`](../research/cli-verb-normalisation.md), decision in [`docs/decisions-log.md`](../decisions-log.md), three-clause rule applied to the Clio surface. No deprecation aliases. **PR1b** ✅ (2026-04-27, v0.13.0, branch `iteration-5/cli-completion-and-ci-fixes`): tab completion (`cfcf completion {bash,zsh,install,uninstall}`), GitHub Actions hardening, iter-6 plan entry 6.24 (Clio FTS title boosting), auto-install completion across all install paths, sentinel-marked rc-file auto-edit, `cfcf doctor` check for completion wiring, manual-setup detection. **PR2 + PR3** ✅ (2026-04-27, branch `iteration-5/user-manual`, COMBINED): (a) `docs/guides/manual.md` (NEW, hub-and-pointer style) + `docs/guides/troubleshooting.md` (NEW, dedicated diagnostics page, full "tab doesn't fire / cfcf server won't start / Clio search returns nothing / oh-my-zsh interactions / worst-case clean reinstall" coverage), (b) `cli-usage.md` Clio section restructured into per-verb subsections + new "Shell completion" section, (c) `installing.md` refresh, README "Start here" callout, (d) `scripts/embed-help-content.ts` generator embeds `docs/guides/*.md` + `docs/api/server-api.md` into `packages/core/src/help-content.generated.ts` (gitignored), (e) `cfcf help [topic]` CLI command with slug + alias resolution + `--list`, (f) `/api/help/topics` + `/api/help/topics/:slug` HTTP routes, (g) web UI Help tab (top-bar `Help` link, two-pane layout, minimal Markdown renderer that rewrites intra-doc `.md` links to navigate the tab in-place), (h) `cfcf doctor` "User manual + help content" check. **PR4** 🟡 (in flight on `iteration-5/help-assistant`): **Help Assistant** (`cfcf help assistant`). Design rewritten 2026-04-27 from the original "Ask the Agent" one-shot concept (now superseded; see [`ask-the-agent.md`](../research/ask-the-agent.md)) to an **interactive cf² role** that runs the user's configured dev agent (claude-code / codex) with a curated system prompt + the embedded help bundle + Clio memory access. Launches in the current shell (no new terminal — the agent's TUI takes over until exit). Read-only by default; mutations gated by the agent's per-command permission prompt. Persists user preferences across sessions via Clio memory Projects (`cfcf-memory-ha` + `cfcf-memory-global`). HA is the **first** of two roles using this architecture; the **Product Architect (PA)** in iter-6 reuses the same machine with a narrower scope (problem-pack creation + spec-iteration). Full design: [`docs/research/help-assistant.md`](../research/help-assistant.md). v1 scope: `cfcf help assistant` CLI subcommand, `helpAssistantAgent` config field (default = `devAgent`), system-prompt assembler, Clio memory schema (Projects, no extra metadata), `cfcf doctor` HA-prereq check. **PR5** (iter-6): Product Architect role + PA-specific Clio memory project + workspace-creation flow. **Full design baseline**: [`docs/research/product-architect.md`](../research/product-architect.md) (written 2026-04-28). Captures: PA's scope (Problem Pack authoring + iteration; hard "no implementation drift" boundary), the discovery → bootstrap → spec-iteration → hand-off flow, the `cfcf-memory-pa` schema (workspace-scoped) + `cfcf-memory-global` (cross-role user preferences PA learns + writes back), the `helpArchitectAgent` config field, the architecture pointer (PA reuses HA's launcher seam but switches to **Pattern B** — `cfcf-docs/AGENTS.md` (codex auto-load) + `cfcf-docs/CLAUDE.md` (claude-code analog) for durable per-repo role guidance + `--cd <repo>/cfcf-docs/` + per-call initial task prompt), and seven open questions tagged for iter-6 kickoff (verb shape `cfcf help architect` vs `cfcf workspace plan`, agent default, model default, `--bootstrap` flag vs separate verb, sentinels in PA-managed `AGENTS.md`/`CLAUDE.md`, hand-off mechanics PA→Architect→loop, cross-role memory merge with HA). Pattern A vs Pattern B comparison lives in [`docs/research/help-assistant.md`](../research/help-assistant.md) §"Alternative approaches considered"; PA's design inherits + extends. Also iter-6: full audit of remaining cfcf top-level verbs (`workspace`, `run`, `review`, `reflect`, `document`, `server`, `config`, `init`, `doctor`, `self-update`, `status`, `resume`, `stop`) with the three-clause lens. |
...
### Iteration 6: CLI parity + New agents + Multi-workspace + Isolation + Observability + Generalization research
...
| 6.2 | ❌ | `cfcf log <workspace-name>` CLI | Iteration history viewer from the CLI -- brings the web History tab to parity on the CLI side. Reads `history.json` |
...
| 6.5 | ❌ | Token / cost tracking | Best-effort per iteration per role, stored on `IterationHistoryEvent` and surfaced in History tab. Adapter-specific parsing (Claude Code emits usage in `--verbose`; Codex has its own shape). Aggregate shown on workspace dashboard |
...
| 6.7 | ❌ | Multi-workspace support in server | Concurrent execution via worker threads or separate child processes; web GUI already supports per-workspace views but server currently assumes one active loop at a time |
...
| 6.9 | ❌ | Optional Cerefox memory backend | Semantic search across workspaces. Not required -- file-based memory is fully functional standalone. Depends on 5.7 (cross-workspace knowledge via Clio) |
...
| 6.15 | ❌ | Clio v2 — sqlite-vec HNSW integration | Second pass at the Clio vector search path. Replace the pure-TS brute-force cosine (`LocalClio.searchSemantic` / `searchHybrid`) with `sqlite-vec`'s native ANN. Requires solving the Bun-on-macOS `loadExtension`-disabled-on-Apple-SQLite issue -- cleanest path ties into item 5.5 (the installer ships a pinned SQLite + sqlite-vec per-platform, `Database.setCustomSQLite(...)` points at it via `CFCF_SQLITE_LIB`). Trigger: corpus sizes climb past ~50k chunks where brute-force latency starts to matter. Benefit: HNSW index is sub-10ms at 1M+ chunks vs. brute-force's linear scan. Data is already in the right shape (embedding BLOB column) — purely additive. |
...
| 6.20 | ❌ | New-version notification in the web UI | The CLI `cfcf self-update` command shipped in 5.5 (a thin wrapper around `bun install -g <new-tarball>` that compares the local installed version against the latest release's MANIFEST). What's left for 6.20 is the **proactive notification** layer: the server polls the releases endpoint at a low cadence (e.g. daily) and surfaces a banner on the top bar when a newer version is available, with a "How to update" tooltip pointing at `cfcf self-update`. Notification only — does NOT auto-apply updates (the user still runs the upgrade explicitly via the CLI). High-level requirement at this stage; full design when scheduled. Pairs with 6.18 (web UI Clio tab) since the same top-bar surface holds both. |
...
| 6.23 | ❌ | Clio chunker re-chunk on embedder switch (`cfcf clio reindex --rechunk`) | **Surfaced 2026-04-27** as a follow-up to the embedder-switch warnings. When the user switches the active embedder to a model with a smaller `recommendedChunkMaxChars` than the existing chunks were sized for, the new model truncates inputs at embed time. Today the warning recommends `cfcf clio reindex --rechunk` but the flag isn't implemented. **Scope**: extend `MemoryBackend.reindex` with a `rechunk: boolean` option; when set, re-runs the chunker per doc with the active embedder's `recommendedChunkMaxChars`, replaces existing live chunks (snapshots them under a new version per doc, same path as content updates), re-embeds. Idempotent — skips docs whose chunk sizes already match. CLI flag on `cfcf clio reindex` + a button on the Server Info page that pairs with the chunk-size warning. **Pairs with 6.22 (version retention)** because both touch the version-snapshot code path. **Why deferred**: the existing pre-flight warning (shipped 2026-04-27) closes the silent-degradation footgun; users have a path forward (`--reindex` for embeddings, manual re-ingest for chunks) until 6.23 lands. |
| 6.22 | ❌ | Clio version retention (Cerefox parity, on-demand cleanup) | **Surfaced 2026-04-27** in the Cerefox-parity audit. Cerefox runs lazy retention on every `cerefox_snapshot_version` call: deletes versions older than `CEREFOX_VERSION_RETENTION_HOURS` (default 48), always keeps the most-recent version, skips `archived=true` versions. cfcf's `clio_document_versions` accumulate forever -- the `archived` column is in the schema but never read. **Scope**: (a) `cfcf clio cleanup-versions [--older-than 168h] [--keep-last 1] [--project <name>] [--dry-run]` + `POST /api/clio/cleanup-versions` -- explicit on-demand cleanup, default thresholds match Cerefox's. Respects `archived=true` (skip). (b) `MemoryBackend.purgeVersions(opts)` returning `{ versionsDeleted, chunksDeleted, docsTouched }`. (c) Optional auto-cleanup via `clio.versionRetentionHours` config + a server-side hook on the snapshot path -- default off (we surface delete actions explicitly). (d) Mark a version protected: `cfcf clio docs versions <doc-id> --archive <version-id>` and `--unarchive`. **Why on-demand instead of lazy**: silent automatic deletion surprises users; the trust story for unattended agent runs is "nothing disappears unless I ask". Cerefox's lazy default is a sensible default for a user-driven KB; cfcf's agent-driven workflow benefits from the explicit form. |
