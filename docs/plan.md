...
| 5.5 | ❌ | Installer + npm-format distribution | **Detailed design: [`research/installer-design.md`](../research/installer-design.md) (2026-04-22, rewritten 2026-04-26 after the `bun --compile` pivot — see `docs/decisions-log.md` 2026-04-26).** Distribution shape: cfcf is a standard npm-format CLI package. Build produces `cfcf-X.Y.Z.tgz` (bundled JS + `package.json` declaring runtime deps). Per-platform native deps (custom-built libsqlite3 with `loadExtension` enabled + sqlite-vec) ship as separate `@cerefox/cfcf-native-<platform>` optional npm packages. User installs via `bun install -g <tarball-URL>` (or via our 30-line `install.sh` wrapper that bootstraps Bun if missing). Bun ≥ 1.3 is a runtime requirement (cfcf uses `bun:sqlite`, `Bun.spawn`, `Bun.file`, `Bun.serve` directly — refactoring to Node-compat would be a major rewrite without much gain). Phased: GitHub Releases tarball for now (cfcf private) → `bun publish @cerefox/cfcf-cli` to npmjs.com when cfcf opens up (5.5b). Release CI: `workflow_dispatch` only; tagging never triggers a release. `cfcf doctor` self-check + `cfcf self-update` CLI ship as part of 5.5. Depends on 5.3 + 5.4 (self-hosted server + embedded assets). Non-interactive provisioning is a deferred follow-up. |
