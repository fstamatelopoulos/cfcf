...
| 6.19 | ❌ | Clio v2 — `cfcf init` Clio onboarding polish | `cfcf init` already prompts for embedder install (added 2026-04-23 under 5.7). This item tightens the onboarding flow: better per-embedder descriptions during the catalogue pick, bandwidth warning before starting the download, resumable download (transformers.js handles caching but we can show a progress bar on stderr using HF's `progress_callback`), post-install verification (`cfcf clio embedder active` runs automatically and prints a "ready" line). Depends on 5.5 (installer) so we can pre-warm the model during install when the user opts in. |
