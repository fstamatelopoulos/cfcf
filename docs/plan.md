...
| 5.7 | 🟡 (designed) | Cross-project knowledge -- codename **Clio** | Persistent agent memory layer: SQLite + sqlite-vec + FTS5 hybrid search, bundled ONNX embedder (bge-small-en-v1.5 default), chunks-first schema ported from Cerefox, `cfcf clio` CLI verbs matching Cerefox MCP tool semantics, auto-ingest on each iteration phase + context-assembly preload. **Research:** [`docs/research/cross-project-knowledge-layer.md`](../research/cross-project-knowledge-layer.md). **Design (implementable):** [`docs/design/clio-memory-layer.md`](../design/clio-memory-layer.md). Scheduled as its own release `v0.8.0` given the scope (new core component). Depends on prerequisite "workspace rename" (cf² `project` → `workspace`) landing first so Cerefox's `Project` semantics aren't overloaded. |
...
| 5.10 | ❌ | Rename cf² `project` → `workspace` (prerequisite for Clio / item 5.7) | **Scheduled before 5.7.** Cerefox's `Project` concept means "grouping of related work" (a domain / ecosystem). cf²'s current `project` means "one managed git repo" — the two collide when Clio lands. Rename resolves it: cf² workspace = one repo, Clio Project = grouping of workspaces (Cerefox-compatible). Scope: `ProjectConfig` type → `WorkspaceConfig`; `cfcf project *` CLI → `cfcf workspace *` (with `project` as deprecated alias for one release); `/api/projects/*` → `/api/workspaces/*` (with alias); `Projects` top-bar link + Project detail page → Workspace everywhere in the web UI; all docs + templates + status messages. Internal file paths `cfcf-docs/` stay unchanged (they don't reference the term). Breaking change for the API; ship standalone so downstream tooling has a clear upgrade point. Design detail in [`docs/design/clio-memory-layer.md`](../design/clio-memory-layer.md) §2. |
