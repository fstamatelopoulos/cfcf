...
| 5.8 | 🟡 | User manual (embedded + CLI + doc) + CLI verb normalisation + tab completion | A canonical user manual covering the full workflow (setup → problem pack → review → run → reflect → document → extend). Four sub-PRs on `iteration-5/cli-verb-normalisation` and follow-up branches. **PR1** ✅ (2026-04-27): CLI verb normalisation — proposal in [`docs/research/cli-verb-normalisation.md`](../research/cli-verb-normalisation.md), decision in [`docs/decisions-log.md`](../decisions-log.md), three-clause rule applied to the Clio surface. No deprecation aliases. **PR1b** ✅ (2026-04-27, branch `iteration-5/cli-completion-and-ci-fixes`): tab completion (`cfcf completion {bash,zsh,install,uninstall}`), GitHub Actions hardening (artifact retention, Node 24, action versions), iter-6 plan entry 6.24 (Clio FTS title boosting), auto-install completion across all install paths (`scripts/install.sh`, `cfcf self-update`, `bun install -g` postinstall hook), sentinel-marked rc-file auto-edit (Bun-style; trust principle preserved via begin/end markers), `cfcf doctor` check for completion wiring, manual-setup detection so we don't fight users who already wired it up themselves. **PR2** ❌: canonical `docs/guides/manual.md` aggregated from the existing workflow + CLI + API docs, plus structured Clio commands section in `cli-usage.md`. **Must include** a **shell-completion** section (rc-file behaviour, sentinel markers, `cfcf completion {install,uninstall}`, opt-out via `--no-rc-edit`, supported shells=bash+zsh on macOS+Linux only) + a **troubleshooting** section covering "tab doesn't work" diagnostics (run `cfcf doctor`; check `~/.zshrc` / `~/.bashrc` for the cfcf-managed block; force reload via `rm ~/.zcompdump*; exec zsh`; what to do under oh-my-zsh / prezto / starship). **PR3** ❌: embed manual + key docs into the installer (`scripts/stage-dist.sh` ships them under `share/cfcf/docs/`); web UI Help tab; `cfcf help <topic>` CLI command that prints topic-scoped sections. Depends on PR2 — embedded files are the manual. **PR4** ❌ (design only inside 5.8; implementation deferred to iter 6): "Ask the agent" — `cfcf ask <question>` / web UI button that launches the configured dev agent with the manual in the system prompt. Iter 6 scope: full audit of remaining cfcf top-level verbs (`workspace`, `run`, `review`, `reflect`, `document`, `server`, `config`, `init`, `doctor`, `self-update`, `status`, `resume`, `stop`) with the same three-clause lens. |
