...
| 5.5b | ❌ | Publish to npmjs.com | When cfcf goes OSS / public: register `@cerefox` org on npmjs.com, run `bun publish` from release.yml, update `install.sh` default `CFCF_BASE_URL`. Install becomes the canonical `bun install -g @cerefox/cfcf-cli`. Tarball-via-GitHub-Releases stays as a fallback for offline / pinned installs. |
...
| 6.16 | ❌ | Clio v2 — audit log wiring | Table shape for `clio_audit_log` already lives in migration 0001 from 5.7. Add writes at every mutation path (ingest / delete / set-project / migrate-project) + every search call. New `/api/clio/audit-log` endpoint with filters (timestamp, event_type, project_id, document_id, actor) + `cfcf clio audit [--filter ...]` CLI. **Reuse plan**: port Cerefox's audit-log query logic from `cerefox/src/cerefox/db/rpcs.sql` + the edge function shape. Match the event_type enum to Cerefox's where possible so the two systems' audit vocabularies align. |
| 6.17 | ❌ | Clio v2 — update-doc API + versioning + soft-delete (Cerefox parity, **prioritised**) | **Surfaced 2026-04-26** during the v0.10.0 dogfood: today `LocalClio.ingest()` dedups by `content_hash` and silently returns `created: false` if you re-ingest byte-identical content; modified content creates a brand-new UUID and orphans the old doc with no version chain. This breaks Cerefox parity (Cerefox's `cerefox_ingest` supports `update_if_exists: true` to match by title and `document_id: <uuid>` for deterministic update — both create archived versions). **Scope**: (a) extend `IngestRequest` + `cfcf clio ingest` + `POST /api/clio/ingest` with `--update-if-exists` and `--document-id <uuid>` flags mirroring Cerefox; (b) `clio_document_versions` writes — snapshot current chunks on every content update, bump `version_number`, archive prior chunks by setting `version_id` to the new version row; (c) `deleted_at` reads everywhere + `DELETE /api/clio/documents/:id` (sets it) + `POST /api/clio/documents/:id/restore` (clears it). Pairs naturally with the audit log (6.16) — same write-path changes touch both. **Reuse plan**: port Cerefox's `cerefox_update_document_content` + `cerefox_soft_delete_document` + `cerefox_restore_document` + version-number bump logic. Adapt Cerefox's retention-config table (`cerefox_requestor_enforcement_config` → `clio_retention_config`) if we want automatic version-cleanup policies. **Why prioritised**: the `MemoryBackend` parity commitment (decisions-log 2026-04-25) means a future `CerefoxRemote` adapter MUST present the same update-doc semantics. Letting the gap linger means caller code accumulates "Clio doesn't support this; if-Cerefox" branches — exactly the drift we committed to avoid. |
