...
| 5.7 | ✅ | Cross-workspace knowledge -- codename **Clio** | Persistent agent memory layer. **Shipped on `iteration-5/clio`** as three logical PRs in one branch (9 commits). PR1: SQLite schema + FTS5 + migrations runner (bun:sqlite, no native deps), markdown chunker (1:1 Cerefox port), LocalClio backend (CRUD + FTS search + content_hash dedup), HTTP `/api/clio/*` + `PUT /api/workspaces/:id/clio-project`, CLI `cfcf clio {search,ingest,get,projects,project,stats}` + `cfcf memory` alias, `cfcf workspace init --project` + interactive Clio Project picker + `cfcf workspace set --project [--migrate-history]`. PR2: embedder catalogue (bge-small default, MiniLM, nomic, bge-base), `@huggingface/transformers` ONNX wrapper with lazy model download to `~/.cfcf/models/`, active-embedder tracking (migration 0002), hybrid RRF + pure-vector search, small-to-big sibling expansion, `cfcf clio embedder {list,active,install,set}`, brute-force cosine in TS (sqlite-vec deferred — see 6.15). PR3: iteration-loop auto-ingest at reflect/architect/decision-log/iteration-summary boundaries (respecting `workspace.clio.ingestPolicy`), `cfcf-docs/clio-relevant.md` context preload, `cfcf-docs/clio-guide.md` agent cue card. **Research:** [`docs/research/cross-project-knowledge-layer.md`](../research/cross-project-knowledge-layer.md). **Design:** [`docs/design/clio-memory-layer.md`](../design/clio-memory-layer.md). **Implementation decisions:** [`docs/research/clio-implementation-decisions.md`](../research/clio-implementation-decisions.md). **Test plan:** [`docs/research/clio-test-plan.md`](../research/clio-test-plan.md). **Release:** v0.9.0 on PR merge. 368+ tests passing. **v2 follow-ups tracked separately as items 6.15-6.19 (sqlite-vec, reindex, audit log, soft-delete + versioning, web Clio tab).** Remote Cerefox backend stays in the backlog (Future Iterations section). |
