...
| 5.7 | 🟡 (researching) | Cross-project knowledge | Agent assessments and lessons learned accumulated across projects, with a query interface for context assembly to pull relevant prior knowledge into new projects. **Research doc: [`docs/research/cross-project-knowledge-layer.md`](../research/cross-project-knowledge-layer.md)** surveys the design space (RAG + agent-memory frameworks + academic CoALA framing), proposes a hybrid Option C: local-default embedded backend (SQLite + sqlite-vec + FTS5 + ONNX embedder) with Cerefox as an opt-in upgrade. Depends on a decision between options before implementation starts. |
