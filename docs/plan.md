...
| 5.7 | ✅ | Cross-workspace knowledge -- codename **Clio** | Persistent agent memory layer: SQLite + sqlite-vec + FTS5 hybrid search, bundled ONNX embedder (bge-small-en-v1.5 default), chunks-first schema ported from Cerefox, `cfcf clio` CLI verbs matching Cerefox MCP tool semantics, auto-ingest on each iteration phase + context-assembly preload. **Research:** [`docs/research/cross-project-knowledge-layer.md`](../research/cross-project-knowledge-layer.md). **Design:** [`docs/design/clio-memory-layer.md`](../design/clio-memory-layer.md). **Implementation decisions:** [`docs/research/clio-implementation-decisions.md`](../research/clio-implementation-decisions.md). Staged as 3 PRs on branch `iteration-5/clio`. **PR1 foundation (shipped commits b9c0c2f, 4993ea8, 287f4a8, 9982575):** SQLite schema + migrations runner (bun:sqlite, FTS5), markdown chunker (1:1 Cerefox port), LocalClio backend (FTS5 search + ingest + dedup), HTTP `/api/clio/*` + `PUT /api/workspaces/:id/clio-project`, CLI `cfcf clio {search,ingest,get,projects,project,stats}` + `cfcf memory` alias, `cfcf workspace init --project` + interactive picker, `cfcf workspace set --project --migrate-history`. 54 new tests (8 db + 12 chunker + 25 LocalClio + 21 HTTP + 9 CLI wiring). **PR2 embeddings (next):** sqlite-vec + onnxruntime-node + bge-small lazy download (binary stays lean) + hybrid RRF search + small-to-big sibling expansion. Handles the Bun-on-macOS loadExtension quirk via `Database.setCustomSQLite()`. **PR3 loop integration:** auto-ingest hooks at reflect/architect/decision-log/iteration-summary boundaries; `cfcf-docs/clio-relevant.md` context preload; `cfcf-docs/clio-guide.md` agent cue card. Release: v0.9.0 once all three PRs ship. |
