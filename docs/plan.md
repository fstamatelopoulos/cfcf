...
| 6.17 | ❌ | Clio v2 — soft-delete + versioning | `deleted_at` (exists in schema; needs reads everywhere + a `DELETE /api/clio/documents/:id` that sets it, `POST /api/clio/documents/:id/restore` that clears it) + `clio_document_versions` writes (snapshot on every content update; bump `version_number`; archive current chunks by setting `version_id` to the new version row). Pairs naturally with the audit log (6.16) because the same write-path changes touch both. **Reuse plan**: port Cerefox's `cerefox_update_document_content` + `cerefox_soft_delete_document` + `cerefox_restore_document` + version-number bump logic. Adapt Cerefox's retention-config table (`cerefox_requestor_enforcement_config` → `clio_retention_config`) if we want automatic version-cleanup policies. |
