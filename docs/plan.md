...
| 5.8 | 🟡 | User manual (embedded + CLI + doc) + CLI verb normalisation | A canonical user manual covering the full workflow (setup → problem pack → review → run → reflect → document → extend). Four sub-PRs on `iteration-5/cli-verb-normalisation` and follow-up branches. **PR1** ✅ (2026-04-27): CLI verb normalisation — proposal in [`docs/research/cli-verb-normalisation.md`](../research/cli-verb-normalisation.md), decision in [`docs/decisions-log.md`](../decisions-log.md), three-clause rule applied to the Clio surface. No deprecation aliases. **PR2** ❌: canonical `docs/guides/manual.md` aggregated from the existing workflow + CLI + API docs, plus structured Clio commands section in `cli-usage.md`. **PR3** ❌: embed manual + key docs into the installer (`scripts/stage-dist.sh` ships them under `share/cfcf/docs/`); web UI Help tab; `cfcf help <topic>` CLI command that prints topic-scoped sections. Depends on PR2 — embedded files are the manual. **PR4** ❌ (design only inside 5.8; implementation deferred to iter 6): "Ask the agent" — `cfcf ask <question>` / web UI button that launches the configured dev agent with the manual in the system prompt. Iter 6 scope: full audit of remaining cfcf top-level verbs (`workspace`, `run`, `review`, `reflect`, `document`, `server`, `config`, `init`, `doctor`, `self-update`, `status`, `resume`, `stop`) with the same three-clause lens. |
