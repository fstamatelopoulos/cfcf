...
| 5.13 | ✅ | Clio — audit log wiring (Cerefox parity) | **Shipped on `iteration-5/clio-update-api` (2026-04-26).** Promoted from 6.16. Schema was already in place from 0001's `clio_audit_log` placeholder. (a) `LocalClio.writeAudit` writes one row per mutation: `create`, `update-content`, `delete`, `restore`, `migrate-project`. Idempotent no-op deletes/restores do NOT write extra rows. Reads (search, get, list) intentionally NOT recorded — the volume would be noisy and the trust story is about writes. Best-effort: write failures are warned to stderr but don't fail the mutation. (b) `cfcf clio audit [--event-type <e>] [--actor <name>] [--project <p>] [--document-id <id>] [--since <iso>] [--limit <n>]` + `GET /api/clio/audit-log` with the same filters. Newest first; mirrors `cerefox_get_audit_log`. Vocabulary aligned with Cerefox's `operation` enum. **Out of 5.13**: retention auto-cleanup of old audit rows + version rows (a small follow-up; pairs with the same write-path edits). |
