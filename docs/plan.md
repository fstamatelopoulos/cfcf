...
| 5.5 | ❌ | Installer script | **Detailed design: [`research/installer-design.md`](../research/installer-design.md) (2026-04-22, expanded 2026-04-25).** Short version: `curl -fsSL https://<host>/install \| bash` → platform detection → download a per-platform tarball from GitHub Releases → sha256 verify → unpack into `~/.cfcf/` → symlink `/usr/local/bin/cfcf` → "press Enter" → exec interactive `cfcf init` (`CFCF_SKIP_INIT=1` skips the prompt + handoff for power users). Tarball is self-contained (~150 MB per platform): Bun-compiled `cfcf` binary + colocated `node_modules/` for the three externalized native deps (`@huggingface/transformers`, `onnxruntime-node`, `sharp`) + a pinned `libsqlite3` built with `SQLITE_ENABLE_LOAD_EXTENSION=1` + `sqlite-vec.<ext>` prebuilt. Bun is **not** a user prerequisite (embedded by `--compile`). The pinned SQLite lets 6.15 load sqlite-vec on every platform including macOS where Apple's system SQLite disables it. Embedder model is downloaded + activated + warmed during `cfcf init` (the installer execs init at the end). Hosting of the install URL is a phased decision (local → public `cfcf-releases` repo → optional `cerefox.org` redirect) — see design doc §2 Q2 + §8. Release automation is new (`.github/workflows/release.yml`), tag-triggered, platform matrix darwin-arm64 / darwin-x64 / linux-x64 for v1; windows-x64 follow-up. Depends on 5.3 + 5.4 so the binary is self-sufficient. Non-interactive provisioning (fleet, devcontainer, CI) is sketched in design-doc §1.1 as a future addition but not part of 5.5; only land if real demand appears. |
...
| 6.20 | ❌ | `cfcf self-update` + new-version notification | Build on top of 5.5: a `cfcf self-update` command that re-runs the installer in upgrade mode against the latest published release. Consumes the same `MANIFEST` file the installer uploads to GitHub Releases — compares the local `~/.cfcf/MANIFEST`'s `cfcf:` line with the latest tag's, prompts for confirmation, then re-runs `install.sh` with `CFCF_VERSION=<newest>`. Upgrade flow already preserves user data per [`research/installer-design.md`](../research/installer-design.md) §3.3. **Web UI surface:** the server polls the releases endpoint at a low cadence (e.g. once a day) and surfaces a banner on the top bar when a newer version is available, with a "How to update" link. Notification logic only — does NOT auto-apply updates. High-level requirement only at this stage; full design when scheduled. Pairs with 6.18 (web UI Clio tab) since the same top-bar surface holds both. |
