...
| 5.8 | ❌ | User manual (embedded + CLI + doc) + CLI verb normalisation | A canonical user manual covering the full workflow (setup → problem pack → review → run → reflect → document → extend). Three surfaces: (a) canonical source `docs/guides/manual.md` aggregated from the existing workflow + CLI + API docs; (b) in-app help panel linked from the web UI `#/server` page (simple markdown render of the same source); (c) `cfcf help <topic>` CLI command that prints topic-scoped sections. Depends on 5.5 (installer) being nearby so the quick-start can realistically say `curl … \| bash` instead of "clone the repo." **Sub-tasks added 2026-04-27**: (d) `docs/guides/cli-usage.md` is missing a proper **Clio commands section** — it lists a few examples in the middle of an unrelated heading but no comprehensive verb-by-verb reference like the `cfcf workspace` / `cfcf run` sections have. Add a structured Clio section that mirrors the rest of the file (one subsection per verb: `search`, `ingest`, `get`, `versions`, `audit`, `delete`, `restore`, `metadata-search`, `metadata-keys`, `docs list`, `docs edit`, `projects`, `project`, `embedder {list,active,install,set}`, `reindex`, `stats`). (e) **CLI verb normalisation review**: today the surface mixes singular/plural conventions (`cfcf clio project|projects` is aliased; `cfcf clio docs` is plural-only with no `doc` alias; some verbs are top-level — `delete`, `restore`, `audit`, `versions`, `metadata-search`, `metadata-keys` — while `docs list` + `docs edit` live under a `docs` namespace). Cerefox's MCP tool names lean on full nouns (`cerefox_get_document`, `cerefox_list_metadata_keys`). Before locking the manual, write a short proposal in `docs/research/cli-verb-normalisation.md` that audits every `cfcf` verb (clio, workspace, server, run, …), proposes a consistent pattern (likely: plural namespace + singular show-one + full-word noun), and lists the alias chain we'd add for one-release backwards compat. Land the rename + manual together so the v0.11 docs ship with the canonical surface. Good small-scope item to close iteration 5. |
| 5.11 | ✅ | Clio — update-doc API + versioning + soft-delete (Cerefox parity, **prerequisite for 5.5b**) | **Shipped on `iteration-5/clio-update-api` (2026-04-26).** Promoted from 6.17. (a) `IngestRequest.documentId` + `IngestRequest.updateIfExists` + `IngestRequest.author` accepted by `LocalClio.ingest`, `POST /api/clio/ingest`, `cfcf clio ingest` (`--document-id`, `--update-if-exists`, `--author`) — mirrors `cerefox_ingest(document_id=…, update_if_exists=…, author=…)`. (b) `clio_document_versions` writes: snapshot current chunks into a new version row, bump `version_number`, archive prior chunks by setting `version_id` to the new row (FTS triggers handle in/out automatically). The version row's `source` column records the OUTGOING author so `cfcf clio versions` answers "who wrote v3?" without JOINing the audit log. (c) `cfcf clio versions <doc-id>` + `GET /api/clio/documents/:id/versions` list archived versions newest-first. (d) `cfcf clio get <doc-id> [--version-id <uuid>]` + `GET /api/clio/documents/:id/content` reconstruct full content for live or archived versions. (e) UNIQUE constraint on `content_hash` dropped so updates whose new content matches another doc's hash don't deadlock (originally migration `0003_relax_content_hash`; subsequently folded into the consolidated `0001_initial.sql` 2026-04-27). (f) `IngestResult.action` (`"created"`/`"updated"`/`"skipped"`) is the source of truth; legacy `created` preserved one release. (g) `cfcf clio delete <id>` (soft-delete) + `cfcf clio restore <id>` + `DELETE /api/clio/documents/:id` + `POST /api/clio/documents/:id/restore` — Cerefox parity. Idempotent restores/deletes; `cfcf clio docs list --include-deleted` opts back into showing tombstones. **Out of 5.11**: retention cleanup of old version rows (small follow-up; pairs with 5.13's audit retention). |
