...
| 5.5b | ❌ | Publish to npmjs.com (after iter-5 stabilises) | When cfcf goes OSS / public: register `@cerefox` org on npmjs.com (✅ done 2026-04-26), run `bun publish` from release.yml, update `install.sh` default `CFCF_BASE_URL`. Install becomes the canonical `bun install -g @cerefox/cfcf-cli`. Tarball-via-GitHub-Releases stays as a fallback for offline / pinned installs. **Blocked on**: 5.8 (user manual), 5.11 (Clio update API + versioning), 5.12 (Clio agent-parity API), 5.13 (Clio audit log). The agent-facing Clio surface must reach Cerefox parity before publishing — otherwise `cf² + Clio` and `cf² + Cerefox` aren't drop-in interchangeable, which breaks the whole `MemoryBackend` story we committed to. |
...
| 5.11 | 🔄 | Clio — update-doc API + versioning + soft-delete (Cerefox parity, **prerequisite for 5.5b**) | **Promoted from 6.17 on 2026-04-26. PR1 of 5.11 lands the update API + version snapshots + read-side versioning; soft-delete mutation API + retention cleanup deferred to a small follow-up.** Today's PR (`iteration-5/clio-update-api`): (a) `IngestRequest.documentId` + `IngestRequest.updateIfExists` + `IngestRequest.author` accepted by `LocalClio.ingest`, `POST /api/clio/ingest`, and `cfcf clio ingest` (`--document-id`, `--update-if-exists`, `--author`) — mirrors `cerefox_ingest(document_id=…, update_if_exists=…, author=…)`; (b) `clio_document_versions` writes wired up — `LocalClio` now snapshots current chunks into a new version row before replacing them, bumps `version_number` sequentially, archives prior chunks by setting `version_id` to the new row (FTS triggers handle in/out of the index automatically); (c) `cfcf clio versions <doc-id>` + `GET /api/clio/documents/:id/versions` list archived versions newest-first; (d) `cfcf clio get <doc-id> [--version-id <uuid>]` + `GET /api/clio/documents/:id/content` reconstruct full content from chunks for live or archived versions; (e) migration `0003_relax_content_hash` drops the UNIQUE constraint on `content_hash` so legitimate updates whose new content matches another doc's hash don't deadlock (dedup-on-create still works via the regular index); (f) `IngestResult.action` (`"created" | "updated" | "skipped"`) is the source of truth — legacy `created` boolean preserved for one release. 18 new core tests + 6 new HTTP tests cover update-by-id, update-by-title (with cross-Project / soft-delete edge cases), chained-update version chains, FTS-archive correctness, hash-collision after migration. **Still open under 5.11**: soft-delete mutation API (`DELETE /api/clio/documents/:id` + `restore`) + retention cleanup of old version rows. **Out of 5.11 (separate items)**: `author`/`requestor` write-attribution beyond the version row's `source` column → 5.12; full audit-log → 5.13. **Why iter-5**: the `MemoryBackend` parity commitment (decisions-log 2026-04-25) means the public agent-facing semantics MUST match Cerefox before we publish 5.5b. |
| 5.12 | ❌ | Clio — agent-parity API surface (Cerefox `AGENT_QUICK_REFERENCE.md` parity, **prerequisite for 5.5b**) | **New 2026-04-26**, scoped from a gap analysis against `../cerefox/AGENT_QUICK_REFERENCE.md`. Cerefox exposes a tight 8-tool MCP surface to agents (`cerefox_search`, `cerefox_ingest`, `cerefox_get_document`, `cerefox_list_versions`, `cerefox_metadata_search`, `cerefox_list_metadata_keys`, `cerefox_list_projects`, `cerefox_get_audit_log`); cfcf Clio currently covers ~3 of those cleanly. This item closes the remaining gap (excluding the audit log which lives in 5.13 and the update API which lives in 5.11). **Scope**: (a) `author` / `requestor` field on every write + read API surface — write-side records who ingested/updated each doc; read-side filters by `requestor` + populates the audit log (5.13); (b) `cfcf clio metadata-search` + `GET /api/clio/metadata-search` — find docs by metadata-only filter (no FTS query required), with optional `updated_since` timestamp filter for the catch-up workflow `metadata_search(metadata_filter={"type":"decision-log"}, updated_since="…")`; (c) `cfcf clio metadata-keys` + `GET /api/clio/metadata-keys` — discover which metadata keys + values exist in the current corpus (mirrors `cerefox_list_metadata_keys`); (d) search-result rendering: surface the **document_id** in a copy-pasteable `[id: <uuid>]` token next to each hit (today the CLI prints a truncated chunk_id), so the "search → ingest with document_id" agent workflow works end-to-end without manual ID lookup. **Reuse plan**: port Cerefox's `cerefox_metadata_search` + `cerefox_list_metadata_keys` SQL + RPC shape directly into `LocalClio` and the `MemoryBackend` interface. Changes the `IngestRequest` shape (adds `author`) — coordinate with 5.11. **Why iter-5**: same parity argument as 5.11. Without this, `cf² + Cerefox` and `cf² + Clio` are not interchangeable — caller code accumulates "if Clio …" branches. |
| 5.13 | ❌ | Clio — audit log wiring (Cerefox parity, **prerequisite for 5.5b**) | **Promoted from 6.16 on 2026-04-26.** Table shape for `clio_audit_log` already lives in migration 0001 from 5.7 (intentional placeholder). Add writes at every mutation path (ingest / update / delete / restore / set-project / migrate-project) + every search call, recording `author`/`requestor` from 5.12. New `/api/clio/audit-log` endpoint with filters (timestamp, event_type, project_id, document_id, actor) + `cfcf clio audit [--filter ...]` CLI mirroring `cerefox_get_audit_log`. **Reuse plan**: port Cerefox's audit-log query logic from `cerefox/src/cerefox/db/rpcs.sql` + the edge-function shape; align the event_type enum with Cerefox's so the two systems' audit vocabularies match. **Why iter-5**: the audit log is the trust layer for unattended agent runs — without it, "the dev agent rewrote the decisions log" is invisible to the user. Pairs with 5.11 (versioning) and 5.12 (author field) — same write-path edits touch all three. Bundling them into one branch is cheaper than three separate efforts. |
