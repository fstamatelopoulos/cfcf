...
| 6.21 | ❌ | `cfcf init --force` reads existing config as defaults | Today, re-running `cfcf init --force` against an already-configured machine prompts with the **hardcoded** defaults (claude-code as dev agent, etc.) — not the user's current values. Should read the existing config (agent picks, models, max iterations, embedder pick, notification settings) and offer those as the defaults so the user can hit Enter through prompts they don't want to change. Same for `cfcf config edit` (verify it already does this; if not, fix it the same way). Small UX polish, captured 2026-04-26 during dogfood install. |
