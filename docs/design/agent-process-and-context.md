...
2. **Formulate iteration plan**: Based on all context, decide what this iteration will accomplish. Update `cfcf-docs/plan.md` with the iteration-specific plan and any changes to the overall plan.
   ...
   **One phase per iteration.** Each iteration is a separate, clean agent process -- no session continuity, no memory carried over except files on disk. The architect (or the dev agent on iteration 1) maps phases to concrete iterations in `plan.md`. Each subsequent iteration executes only the **next pending chunk** and marks completed items `[x]` with a short note before exiting. This is what makes the judge's per-iteration assessment meaningful and the loop resumable after a pause. The dev agent's `CLAUDE.md` / `AGENTS.md` (generated by `context-assembler.generateInstructionContent()` every iteration) carries this discipline into every run, independent of what the user's `process.md` copy says.

3. **Execute**: Write code, run tests, debug, iterate internally. The agent works autonomously on the single scheduled chunk until it decides this iteration's work is complete.
