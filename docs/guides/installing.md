...
> **A note on Anthropic's third-party-harness policy** (item 6.28): cfcf's iteration loop is the textbook unattended-harness pattern Anthropic's Jan–Apr 2026 clarification targets. The interactive Product Architect (`cfcf spec`) and Help Assistant (`cfcf help assistant`) roles stay on Claude Code by default — they're within Anthropic's allowed-interactive scope. For the unattended roles (dev / judge / reflection / documenter), the recommended setup uses `codex`, `claude-code-ollama`, `opencode-ollama`, or `opencode`. Read [`anthropic-policy.md`](anthropic-policy.md) (also `cfcf help anthropic-policy` after install) before running `cfcf init` so you can pick adapters with the policy in mind. cfcf surfaces a warning at init time if you've picked `claude-code` for an unattended role; the choice is yours but the warning is there so you don't pick it accidentally.
