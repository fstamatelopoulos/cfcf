...
6. **Surfaces an Anthropic harness-policy warning** (item 6.28) when `claude-code` is picked for any unattended role (dev / judge / reflection / documenter, plus architect when `autoReviewSpecs=true`). The warning is informational — your config still saves. PA / HA / manually-invoked SA on `claude-code` do NOT trigger the warning. See `cfcf help anthropic-policy` for the full breakdown.
